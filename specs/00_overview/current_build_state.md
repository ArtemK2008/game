...
- The world map now uses one authored presentation surface instead of a small bottom strip:
  ...
  - authored-map node presentation is now compact and icon-first:
    ...
    - current and selected nodes use restrained visual highlights instead
    - hit targets stay generous even though the visible node art is smaller and cleaner
